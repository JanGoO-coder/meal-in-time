...
onMounted(() => {
    userStore.checkAuth('/pending-orders')
    userStore.getToken()
...
<template>
    <div class="container py-16">
        <div class="flex flex-row justify-between items-center">
            <h1 class="display-6 pb-8">Pending Orders</h1>
        </div>
        <div>
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                <li class="nav-item me-2" role="presentation">
                    <button class="nav-link active" id="pills-home-tab" data-bs-toggle="pill"
                        ...
                        aria-selected="true">Take Away Pending Orders</button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="pills-profile-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-profile" type="button" role="tab" aria-controls="pills-profile"
                        aria-selected="false">Table Booking Pending Orders</button>
                </li>
            </ul>
            <div class="tab-content" id="pills-tabContent">
                    ...
                    <table class="table text-sm md:text-lg">
                        ...
                        <tbody>
                            <tr v-for="(takeAway, index) in orderStore.takeAwayOrders" :key="index"
                                v-show="takeAway.data.orderStatus == 'Pending'">
                                <th scope="row" class="text-center">{{ index + 1 }}</th>
                                <td scope="row" class="truncate text-ellipsis max-w-[5ch] md:max-w-[48ch]">
                                    <ul class="list-group">
                                        <li v-for="(item, i) in takeAway.data.order" :key="i"
                                ...
                                <td class="text-center">{{ takeAway.data.orderStatus == null ? "Pending" :
                                        takeAway.data.orderStatus
                                }}</td>
                                <td class="text-center">{{ takeAway.data.totalAmount }}</td>
                                <td class="flex justify-end gap-2">
                                    <button class="btn btn-success"
                                        @click="orderStore.updateTakeAwayOrderStatus(takeAway.key, 'Received')">
                ...
                <div class="tab-pane fade" id="pills-profile" role="tabpanel" aria-labelledby="pills-profile-tab">
                    <table class="table text-sm md:text-lg">
                        ...
                        <tbody>
                            <tr v-for="(tableBook, index) in orderStore.tableBookingOrders" :key="index"
                                v-show="tableBook.data.orderStatus == 'Pending'">
                                <th scope="row" class="text-center">{{ index + 1 }}</th>
                                <td scope="row" class="truncate text-ellipsis max-w-[5ch] md:max-w-[48ch]">
                                    <ul class="list-group">
                                ...
                                <td class="flex justify-end gap-2">
                                    <button class="btn btn-success"
                                        @click="orderStore.updateTableBookingOrderStatus(tableBook.key, tableBook.data.tableId, 'Received')">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                        </svg>
